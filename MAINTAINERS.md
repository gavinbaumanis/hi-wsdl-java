...
**This 1.6.4.1 line (`1.6.4.1-SNAPSHOT`):** Java **8**, committed **`javax`** generated types, **14** primary HI B2B **`@WebServiceClient`** services (no **`wsimport`** / XJC in this POM). **`hi-b2b-client`** **1.6.4.1** uses **`hi-wsdl`** at the **same version** (**`${project.version}`**) - **`mvn install`** here before an unpublished client **`verify`**. GA **1.6.4.1** pairs ship to Maven Central together. Type regeneration for new HI releases is on the **`java-11`** / **1.7.1.1** line.
...
- **No source regeneration** on **`1.6.4.1`** - no licensed MCA **`wsdls/xml`** tree and no **`-Pregenerate-sources`** profile. **`src/main/java`** and **`hi_override/`** are committed as-is.
...
Git/SCM settings for **`maven-release-plugin`** live in **`pom.xml`** properties (**`scm.repo.url`**, **`release.*`**). Tags default to **`{artifactId}-{version}`** (e.g. **`hi-wsdl-1.6.4.1`**).
...
```text
mvn -B "-Prelease" release:prepare release:perform -DreleaseVersion=1.6.4.1 -DdevelopmentVersion=1.6.4.2-SNAPSHOT -Dtag=hi-wsdl-1.6.4.1
```

Replace versions and **`-Dtag`** for the line you are on. Cut **`hi-wsdl`** and **`hi-b2b-client`** at the **same** GA (this line: **`hi-wsdl-1.6.4.1`** and **`hi-b2b-client-1.6.4.1`**). Omit **`-D...`** only if you accept interactive prompts.

**After success:** confirm the artifact on Central; repeat on the paired types/client repo. No extra Git steps unless push failed (then **`git push origin master`** and **`git push origin <tag>`**).
