...
This **1.6.4.1** line is Java **8** / **`javax`**, with **committed** generated types and **14** standard HI B2B services (no **`wsimport`** in the build).
...
This repository builds **`1.6.4.1-SNAPSHOT`** on the Java **8** / **`javax`** line. **`hi-b2b-client-java`** declares **`au.gov.nehta:hi-wsdl`** at **`${project.version}`** - install this types JAR **first** when both checkouts are unpublished:

```text
# 1) hi-wsdl (1.6.4.1-SNAPSHOT - this repository)
mvn -B "-Dgpg.skip=true" clean install

# 2) hi-b2b-client (1.6.4.1-SNAPSHOT - matching version)
mvn -B "-Dgpg.skip=true" clean verify
```

If Maven warns that a **GA** POM is missing (for example **`1.6.4.1`** before Central publish), clear stale **`au/gov/nehta/hi-wsdl`** entries in your **local Maven repository** (folders with only **`.lastUpdated`** files) and reinstall the SNAPSHOT. **`mvn clean`** in one project does not clear the local repository cache.
