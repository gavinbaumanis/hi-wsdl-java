...
| ------- | ---- | --------- | ---------------- |
| **1.6.4.1** | **8** | **`javax.*`** / EE4J **`jaxws-rt` 2.3.x** at runtime in consumers | Standard HI B2B - **14** `Service` stubs |
| **1.7.1.1** | **11** | **Jakarta** / EE4J **`jaxws-rt` 4.0.x** at runtime in consumers | Full MCA - **26** `Service` stubs |

This **1.7.1.1** line is Java **11** / **Jakarta**, full MCA (**26** `Service` stubs).
...
Align **`hi-wsdl`** and **`hi-b2b-client`** at the **same version** when both are on the classpath (**1.6.4.1** with **1.6.4.1**, **1.7.1.1** with **1.7.1.1**).

## Local development (SNAPSHOT)

This repository builds **`1.7.1.1-SNAPSHOT`**. **`hi-b2b-client-java`** declares **`au.gov.nehta:hi-wsdl`** at **`${project.version}`** — install this types JAR **first** when both projects are unpublished:

```text
# 1) hi-wsdl (1.7.1.1-SNAPSHOT - this repository)
mvn -B "-Dgpg.skip=true" clean install

# 2) hi-b2b-client (1.7.1.1-SNAPSHOT - matching version)
mvn -B "-Dgpg.skip=true" clean verify
```

If Maven warns that a **GA** POM is missing (for example **`1.7.1.1`** before Central publish), clear stale **`au/gov/nehta/hi-wsdl`** entries in your **local Maven repository** and reinstall the SNAPSHOT.
...
| Codegen | **26** committed `Service` stubs; **`-Pregenerate-sources`** for maintainer refresh only | **`1.7.1.1`**: in-repo **`wsimport`** (26 executions) by default; optional **`-Phi-wsdl-artifact`** uses matching **`hi-wsdl`** from Maven Central |
